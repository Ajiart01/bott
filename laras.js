        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isNumber(user.rupiah)) user.rupiah = 0
            ...
            const isROwner = [conn.decodeJid(conn.user.id), ...owner.map(([number, isCreator, isDeveloper]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            if (e.name) for (let [jid] of owner.filter(([number, isCreator, isDeveloper]) => isDeveloper && number)) {
            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        let pp = 'https://telegra.ph/file/32ffb10285e5482b19d89.jpg'
                        let ppgc = 'https://telegra.ph/file/32ffb10285e5482b19d89.jpg'
                        try {
                ...
                break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Telah naik jabatan dari Rakyat biasa jadi Pejabat di grub ini.```')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Telah dipecat dari jabatannya dan kini menjadi rakyat biasa.```')
...
// const myGroupData = await conn.groupMetadata('120363080752343366@g.us')
// if (!m.sender.includes(myGroupData.participants) {
// m.reply(`KAMU TERTEDEKSI BELUM JOIN KE CHANNEL RESMI KAMI. SILAHKAN JOIN KE CHANNEL KAMI AGAR BISA MENGGUNAKAN BOT.


// LINK CHANNEL:
// https://linkpoi.ga/CHANNELRETHA
...
conn.ws.on('CB:call', async function callUpdatePushToDb(json) {
      ...
      //  await conn.reply('120363026481498527@g.us', `*NOTIF CALLER BOT!*\n\n@${callerId.split`@`[0]} telah menelpon *Raisaaa*\n\n ${callerId.split`@`[0]}\n`, null, { mentions: [callerId] })
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        created: '⛔ *AKSES DI TOLAK* ⛔\n\nPerintah ini hanya pengguna yang sudah membuat base\nContoh: #createbase Aine',
        group: '⛔ *AKSES DI TOLAK* ⛔\n\nfitur ini hanya bisa digunakan di grub! klo gapunya grub join aja ke grub Raisa: https://chat.whatsapp.com/DoT0a1xivzllbsWIE8XEkO',
        private:  '⛔ *AKSES DI TOLAK* ⛔\n\nfitur ini hanya bisa di gunakan di chat pribadi',
        admin: '⛔ *AKSES DI TOLAK* ⛔\n\nNte bukan pejabat grub!,Silahkan minta jendral buat naik jabatan.',
        botAdmin: '⛔ *AKSES DI TOLAK* ⛔\n\nJadikan Raisa sebagai admin untuk menggunakan fitur ini.',
        unreg: 'Silahkan daftar untuk menggunakan fitur ini dengan cara mengetik:\n\n*#daftar nama.umur*\n\nContoh: *#daftar laras.16*',
