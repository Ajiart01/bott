...
let handler = async (m, { isPrems }) => {
    let time = db.data.users[m.sender].lastmonthly + 2592000000
  if (new Date - db.data.users[m.sender].lastmonthly < 2592000000) throw `Anda sudah mengklaim, klaim bulanan ini\ntunggu selama ${msToTime(time - new Date())} lagi`
      // conn.reply(m.chat, `Anda sudah mengklaim dan mendapatkan :`, m)
       ...
       // global.db.data.users[m.sender].pet += 3
         conn.sendButton(m.chat, `Selamat kamu mendapatkan:\n\n+${isPrems ? prem : free} Exp\n+${isPrems ? moneyprem : moneyfree} Money\n+${isPrems ? limitprem : limitfree} Limit`, wm, 'https://telegra.ph/file/82baed862e025b3c70745.jpg', [['ATM ALL', '.atmall']], m, { asLocation: true })
        // conn.reply(m.chat, `Selamat kamu mendapatkan:\n\n+${isPrems ? prem : free} Exp\n+${isPrems ? moneyprem : moneyfree} Money\n+${isPrems ? limitprem : limitfree} Limit`, m)
        db.data.users[m.sender].lastmonthly = new Date * 1
    }
