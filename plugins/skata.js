			...
			room.waktu = setTimeout(() => {
				conn.reply(m.chat, `Waktu jawab habis\n@${room.curr.split`@`[0]} tereliminasi`, room.chat).then(_ => {
					...
					if (room.player.length == 1 && room.status == 'play') {
						...
						conn.sendButton(m.chat, `@${member[0].split`@`[0]} Menang`, `+${room.win_point}XP`, '', [['Sambung Kata', '.skata'], ['Top Player', '.topskata']], room.chat, { contextInfo: { mentionedJid: member } }).then(_ => {
							delete conn.skata[id]
							return !0
						})
					}
...
Dan hanya bisa dimainkan oleh player yang terdaftar`.trim()
			room.chat = await conn.sendButton(m.chat, caption, `Ketik\n*${usedPrefix + command}* untuk join/ikut\n*${usedPrefix + command} start* untuk memulai`, '', [['Ikut', `${usedPrefix}skata`]], m, { mentions: conn.parseMention(caption) })
		}
	} else {
		conn.skata[id] = {
			id,
			...
			filter,
			genKata,
			chat: conn.sendButton(m.chat, game, rules, '', [['Bergabung', `${usedPrefix + command}`]], m),
			waktu: false
