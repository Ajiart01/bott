...
let handler = async (m, { conn, usedPrefix, text }) => {
  let time = db.data.users[m.sender].lastmining + 3600000
  if (new Date - db.data.users[m.sender].lastmining< 3600000) throw `Anda sudah mengambil hadiah\ntunggu selama ${msToTime(time - new Date())} lagi`
	...
	db.data.users[m.sender].limit += limitt * 1
	db.data.users[m.sender].exp += xpee * 1
	db.data.users[m.sender].money += moneyy * 1
	db.data.users[m.sender].exp -= 500
	db.data.users[m.sender].money -= 500
	db.data.users[m.sender].lastmining = new Date * 1
    // m.reply(`Selamat kamu mendapatkan : \n+${xpee} XP\n+${moneyy} Money\n+${limitt} Limit\n\nDipotong pajak :\n-500 XP\n-500 Money`)
    conn.sendButton(m.chat, `Selamat kamu mendapatkan : \n+${xpee} XP\n+${moneyy} Money\n+${limitt} Limit\n\nDipotong pajak :\n-500 XP\n-500 Money`, wm, 'https://telegra.ph/file/1487031126e21907c3054.jpg', [['ATM ALL', '.atmall']], m, { asLocation: true })
    setTimeout(() => {
