...
const { preSudo } = require('../lib/myfunc')
let handler = m => m
...
handler.before = async function (m) {
		...
		room.waktu = setTimeout(() => {
			...
			this.reply(m.chat, `Waktu jawab habis\n@${room.curr.split`@`[0]} tereliminasi -${lose_skata} MMR${room.killer ? `\n@${room.killer.split`@`[0]} +${win_skata} MMR` : ''}`, room.chat).then(_ => {
				...
				if (member.length == 1 && room.status == 'play') {
					this.sendButton(m.chat, `@${member[0].split`@`[0]} Berhasil bertahan`, `+${room.win_point}XP`, '', [['Sambung Kata', '.skata'], ['Top Player', '.topskata']], room.chat, { mentions: conn.parseMention(`@${member[0].split`@`[0]} Berhasil bertahan`) }).then(_ => {
					...
					room.new = true
					who = room.curr
					preSudo('nextkata', who, m).then(async _ => {
	...
	if (room.curr == m.sender) {
		if (/nyerah/i.test(m.text)) {
			...
			if (member.length == 1 && room.status == 'play') {
				await this.sendButton(m.chat, `@${member[0].split`@`[0]} Berhasil bertahan`, `+${room.win_point}XP`, '', [['Sambung Kata', '.skata'], ['Top Player', '.topskata']], room.chat, { mentions: conn.parseMention(`@${member[0].split`@`[0]} Berhasil bertahan`) })
			...
			room.diam = true
			who = room.curr
			let msg = await preSudo('nextkata', who, m)
