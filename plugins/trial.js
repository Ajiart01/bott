...
let handler = async (m, { conn, text, isOwner, groupMetadata }) => {
    ...
    if (db.data.users[m.sender].joinlimit == 0) return m.reply('Maaf kamu sudah tidak bisa menggunakan free join..\nSilahkan chat owner untuk reset limit join')
    db.data.users[m.sender].joinlimit -= 1
    let id = m.chat
   // let groupMetadata = await conn.groupMetadata(m.chat)
    let res = await conn.groupAcceptInvite(code)
    expired = Math.floor(Math.min(999, Math.max(3, isOwner ? isNumber(expired) ? parseInt(expired) : 0 : 3)))
    m.reply(`Berhasil join grup ${res} selama ${expired ? ` selama ${expired} hari` : ''}`)
    conn.reply(`Bot telah di undang di group: ${groupMetadata.subject}\nCode ID: ${res}`, `6285163999446@s.whatsapp.net`)
    conn.sendMessage(`6281310253704-1616818532@g.us`, {text: `Bot telah di undang ke group: ${code}\nCode ID: ${res} secara trial.` }) //made by Muhammad Ridwan Reynaldy  
    setTimeout(() => {
