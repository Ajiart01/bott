...
let handler = async (m, { conn, text, usedPrefix }) => {
  ...
  var now = new Date() * 1
  db.data.users[hl[0]].banned = true
  if (now < db.data.users[hl[0]].bannedDate) db.data.users[hl[0]].bannedDate += jumlahHari
  else db.data.users[hl[0]].bannedDate = now + jumlahHari
  conn.reply(m.chat,`*TERBANNED*\n\nKamu di banned *@${hl[0].split('@')[0]}* selama *${hl[1]} hari*.\n\n*Banned : ${msToDate(db.data.users[hl[0]].bannedDate - now)}*`,m,{ contextInfo: { mentionedJid: [hl[0]] } })
