  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      try {
        ...
        stiker = await sticker(false, out, `Made with ❤ by`, nama)
      } catch (e) {
        console.error(e)
        if (!stiker) stiker = await sticker(img, false, `Made with ❤ by`, nama)
      }
    } else if (args[0]) {
      if (isUrl(args[0])) stiker = await sticker(false, args[0], `Made with ❤ by`, nama)
