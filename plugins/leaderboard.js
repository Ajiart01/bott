...
• *${len} TOP SALDO ATM BRI* •
Kamu: *${usersBank.indexOf(m.sender) + 1}* dari *${usersBank.length}*

${sortedMoney.slice(0, len).map(({ jid, money }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : 'wa.me/'}${jid.split`@`[0]} *Rp ${money}*`).join`\n`}

• *${len} Crazy Rich RaisaBot* •
...
${sortedExp.slice(0, len).map(({ jid, exp }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : 'wa.me/'}${jid.split`@`[0]} *${exp} Exp*`).join`\n`}

• *Limit Leaderboard Top ${len}* •
Kamu: *${usersLim.indexOf(m.sender) + 1}* dari *${usersLim.length}*

${sortedLim.slice(0, len).map(({ jid, limit }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : 'wa.me/'}${jid.split`@`[0]} *${limit} Limit*`).join`\n`}

• *Level Leaderboard Top ${len}* •
Kamu: *${usersLevel.indexOf(m.sender) + 1}* dari *${usersLevel.length}*

${sortedLevel.slice(0, len).map(({ jid, level }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : 'wa.me/'}${jid.split`@`[0]} *Level ${level}*`).join`\n`}
