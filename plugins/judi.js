...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
        ...
        if (new Date - db.data.users[m.sender].judilast > 5000) {
        db.data.users[m.sender].judilast = new Date * 1
        let randomaku = `${Math.floor(Math.random() * 101)}`.trim()
        let randomkamu = `${Math.floor(Math.random() * 75)}`.trim()                //hehe Biar Susah Menang :v
        let Aku = (randomaku * 1)
        let Kamu = (randomkamu * 1)
        let count = args[0]
        count = count ? /all/i.test(count) ? Math.floor(db.data.users[m.sender].money / buatall) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
        ...
        if (db.data.users[m.sender].money >= count * 1) {
            ...
            if (Aku > Kamu) {
                ...
                db.data.users[m.sender].money += count * 2
