...
let handler = async function (m, { conn, text, usedPrefix }) {
  ...
  let __waktuh = (new Date - db.data.users[m.sender].reglast)
   let _waktuh = (+ 86400000 - __waktuh)
   let waktuh = clockString(_waktuh)
   if (new Date - db.data.users[m.sender].reglast > + 86400000) {
   user.reglast = new Date * 1
  user.name = name.trim()
  user.age = age
  user.regTime = + new Date
  user.registered = true
  let expnye = db.data.users[m.sender].exp += 10000
  let uang = db.data.users[m.sender].money += 20000
  let limitnye = db.data.users[m.sender].limit += 20
